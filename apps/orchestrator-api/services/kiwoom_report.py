...
def _new_buy_ideas(news: str, kst_date: str) -> str:
    ...
    try:
        ...
        sysmsg = (
            ...
            "chain / adjacent beneficiaries surfaced by today's news. Do NOT use a "
            "Markdown table. Present the 5 picks as a numbered list — one block per "
            "pick: a bold header line 'Stock (Ticker)', then on the next lines "
            "'Buy thesis:', 'Catalyst/News:', and 'Risk:' as short labelled sentences, "
            "then a 2-3 sentence paragraph going deeper. Each Buy thesis MUST cite the "
            "SPECIFIC catalyst/news driving it (paraphrase the real headline) and give "
            "a concrete, DIFFERENTIATED thesis — never generic, never the same opening "
            "twice. Do NOT fabricate "
        ...
        if not bad:
            ...
            try:
                ko_sys = (
                    ...
                    "Section 7 (오늘의 신규 매수 추천 5종목) is a numbered prose list, NOT a "
                    "table — keep it as prose; do NOT convert it into a table. Translate "
                    "BUY/HOLD/SELL → 매수/보유/매도. NEVER abbreviate or "
