...
def build_kiwoom_report(db, trace_id: str) -> dict:
    ...
    try:
        ...
        sysmsg = (
            ...
            "- Section 5 (Opportunities): summarize the best setups across the watchlist "
            "in prose, weaving in 수급(외국인/기관), 공매도 trend, and futures positioning "
            "(선물) where provided. Do NOT output a BUY/SELL/HOLD action table — the daily "
            "buy recommendations are produced separately (see the Korean recommendation "
            "section appended later). (Options trading values are not available.)\n"
