...
def build_master_report(db, trace_id: str) -> dict:
    ...
    try:
        ...
        sysmsg = (
            ...
            "⚠ FORMAT RULE: DO NOT output ANY Markdown table (no '|'-delimited tables) "
            "anywhere in this report. Present every section — including catalysts, "
            "broker calls, and final recommendations — as prose and bulleted (•) lists.\n"
            "Rules:\n"
            ...
            "table) — then a 2-3 paragraph synthesis: what the brokerages collectively "
            "favour, notable target-price moves, and cite SPECIFIC recent reports by "
            "firm (e.g. '미래에셋: <title>', '한국투자: <title>'). If no broker data was "
            "provided, write one line saying so. NEVER invent a target price or a "
            "firm's call.\n"
            "- Section 6: final recommendations as a BULLET LIST (no table) — one "
            "bullet per stock: '• <Stock> — <BUY/HOLD/SELL>, confidence <…>: <reason>'; "
            "let the brokerage consensus INFORM these; THEN '### Top Conviction Ideas' — "
            "the 2-3 strongest event-driven setups with the entry-before / "
        ...
        if not bad:
            ...
            try:
                ko_sys = (
                    ...
                    "Preserve ALL Markdown structure (headings, bullets). Do NOT add any "
                    "Markdown table — this report intentionally has none; keep everything "
                    "as prose and bulleted (•) lists.\n"
