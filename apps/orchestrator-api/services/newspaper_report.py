...
def build_newspaper_report(db, trace_id: str) -> dict:
    ...
    try:
        ...
        ssys = (
            ...
            "'신문별 뉴스' 섹션은 따로 추가되니 쓰지 마세요. 어떤 마크다운 표(| 로 구분된 표)도 "
            "절대 출력하지 마세요 — 시세 표·추천 표·일정 표 모두 금지. 모든 내용은 산문과 "
            "글머리표(•) 목록으로만 작성하세요. "
            ...
            "- ## 5. 추천: 표를 만들지 마세요(마크다운 표 금지). 각 관심종목을 한 단락씩 "
            "산문으로 작성하되, 종목명(STOCK STATS의 한국어 이름 그대로, 영어 금지)으로 시작해 "
            "의견(매수/보유/매도)을 분명히 밝히고, STOCK STATS의 일일·주간 등락/거래량 수치를 "
            "그대로 인용하며(지어내지 말 것), 구체적 핵심 근거와 출처를 [출처: <매체>] 형식으로 "
            "답니다('기술적 분석' 같은 막연한 표현 금지). 각 종목 단락은 서로 다른 문체로(템플릿 "
            "복붙 금지) ① 구체적 뉴스+출처, ② 일일·주간 거래량·가격 추세를 실제 숫자로, "
            "③ 외국인/기관 수급, ④ 촉매+시점을 5-7문장으로 담으세요.\n"
