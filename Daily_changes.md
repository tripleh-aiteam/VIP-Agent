...
### [PM3] Algo 2 first live wins + who-traded filter + Algo-1-style board view

- **What:** Algorithm 2's first real auto round trips landed: 2/2 wins, +₩1,774,725 net (SKH ×2, take 0.4 → net ~+0.19% each; boss's "+846,640 (0.2%)" rows confirmed fee-net by recomputation). (1) Both trade histories gained a **who-traded dropdown** (전체/👤/🤖/⚡/🛡️) with live per-actor totals (sells · wins · realized ₩, 수수료 차감 후 labels on all profit columns). (2) Fleet loaded server-side: the real 20-company board + AK홀딩스 = 21 codes via /scalp/params (the first attempt grabbed an alphabetical KRX list from /paper-desk/stocks — WRONG source, corrected from position_guard.GUARD_CODES; note /paper-desk/stocks ≠ watchlist). (3) Board made Algo-1-style: only the 2 mains show; other 19 collapse into a 👁️ watching strip (machine trades them all; a BUY auto-pops the card); chips to open, per-card ✕ to hide, "✕ SK·삼성만 보기" one-click reset; mains pinned first, then LONG, then near-trigger; every card's name = 📈 inline 5-min chart toggle. (4) `status()` price fan-out parallelized — 21 serial Kiwoom calls (~4s+) were piling up under the page's 4s poll.
- **Files:** [services/scalp_trader.py](apps/orchestrator-api/services/scalp_trader.py), [scalp-desk.tsx](apps/admin-dashboard/src/app/testing/scalp-desk.tsx), [desk.tsx](apps/admin-dashboard/src/app/testing/desk.tsx)
- **Next:** verify the parallel-status build lands (status was timing out on the serial build); watch multi-position exposure with 21 stocks (consider a max-open cap like Algo 1); fix the ⭐ 전체 추가 button's data source (currently /paper-desk/stocks = wrong list — should use the board watchlist).
...
### [PM4] ⚡ ALGORITHM 2 — the ripple scalper (new trading way + 2-algorithm landing)

- **What:** Boss spec: a low-risk scalper — buy when price STARTS rising, sell the small win (his example +3,000 on SKH), re-buy if it keeps rising; after a sell that falls, WAIT for the next upturn; after a buy that falls, HOLD the dip and cut only at −1%; no 1-hour holds. Built as a parallel system to Algorithm 1 (which is untouched).
- **Backend:** [services/scalp_trader.py](apps/orchestrator-api/services/scalp_trader.py) — `scalp_state`/`scalp_trades` tables; 15s heartbeat (scheduler job `scalp-heartbeat`); upturn = ≥+0.10% off the 10-min low + consecutive rises, never chases past +0.45%; exits TAKE (+0.4% default — fees are 0.23% round-trip so 0.2% gross would LOSE) / STOP (−1%) / EOD flat 15:18 / EXTERNAL reconcile (same shared-book lesson as auto_trader); buffer cleared after each close so re-buys need a fresh rise; dials via `/paper-desk/scalp/params` (take 0.3–1.0, stop 0.5–1.5, size 5–20% of cash, codes ≤6). Guard skips scalp-held tickers. Manual auto-sell = paper-desk LIMIT SELL filled by the same heartbeat (works with page closed).
- **Frontend:** [testing/page.tsx](apps/admin-dashboard/src/app/testing/page.tsx) is now a LANDING (nothing opens by default): two algorithm cards w/ short explanations → Algo 1 (semi/auto/manual) · Algo 2 (auto/manual). New [scalp-desk.tsx](apps/admin-dashboard/src/app/testing/scalp-desk.tsx) + `/testing/scalp/[mode]`: AUTO = ON/OFF + dials + per-stock WAIT/LONG cards (entry/take/stop lines, bounce readout) + rules in friend language; MANUAL = Kiwoom order-book ladder (2s), 5-min candle chart (15s), BUY/SELL market buttons, ⏳ 자동 매도 (sell-all limit at boss's price + cancel), recent fills; both modes share today's record + trade log. Same paper account/cash as Algo 1, separate scoreboard.
- **Verified:** state machine dry-tested (fires on bounce off low, refuses falls, refuses late chases); tables created on prod DB; tsc + next build clean.

### [PM5] Algorithm 2 round 2 — three-voice buy check + stock picker

- **What:** Boss asked (1) does Algo 2 read the Kiwoom graph / order book / partner stock? Answer was price-stream-only → upgraded: a BUY now needs **three agreeing voices** — ① price upturn (as before) ② Kiwoom order book (`order_book` imbalance ≥ −0.10 — sellers must not dominate; fail-open when book unavailable) ③ partner stock (⑨ peer layer, SKH↔삼성전자 corr 0.83 — no buy while the partner drops >0.15%/60s). Gate results logged + stored on the opened trade. (2) Stock picker like Algo 1: dropdown from the watchlist + free search by KR/EN/code, chips with ✕, mains SK하이닉스·삼성전자 pinned on top (backend `set_params` enforces order), cap 6→8 codes. Manual-mode tabs follow the list.
- **Files:** [services/scalp_trader.py](apps/orchestrator-api/services/scalp_trader.py), [scalp-desk.tsx](apps/admin-dashboard/src/app/testing/scalp-desk.tsx)
...
- **[14:00] UX round (boss's live feedback):** Reset now clears history (build-test rows had polluted his record); typing/focus fixed (Sect component was defined inside the page → remount per keystroke); native datalist replaced with a custom autocomplete (starts-with matches first — "sk" lists ALL SK○○ — scrollable ≤60) + a browsable 전체 종목 dropdown (KOSPI/KOSDAQ groups); order box shows a full live quote strip (시가/현재가±%/고가/저가, refreshes 5s); names for ALL stocks from a new `krx_stocks` table (2,873 rows via FinanceDataReader — boss saw "095570" instead of AJ네트웍스) + GET /paper-desk/stocks; order errors now surface the server message + redeploy hint. All prod-verified (AJ네트웍스 quote complete).
